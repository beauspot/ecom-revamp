...
export class ProductCategoryService {
  constructor(
    ...
    private productCatModel: typeof ProductCategoryModel
  ) {}

  createCategoryService = async (category: ProductCategoryInterface) => {
  ...
  getCategoryService = async (category: string) => {
    const categoryExists = await this.productCatModel.findById({
      _id: category,
    });
  ...
  getAllCategoryService = async (): Promise<ProductCategoryInterface[]> => {
