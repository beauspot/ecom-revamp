...
create_product = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const newProduct = await this.productService.createProductService(req.body);
...
get_all_products = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const paginatedProducts = await this.productService.getAllProductsService(filterOpt);

    const transformedProducts = plainToInstance(ProductDTO, paginatedProducts.page, {
      excludeExtraneousValues: true,
      exposeUnsetFields: false
    });
...
// getting a single product
 getASingleProduct = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    //console.log(id);
     const productDataID = await this.productService.getSingleProductService(id);
...
updateSingleProduct = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    validateMongoDbID(id);
    // console.log(id);
    const updateProduct = await this.productService.updateProductService(id, req.body);
...
uploadImageCtrl = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      const findProduct = await this.productService.uploadImageService(id, files);
